...
if(!class_exists('uwpqsfprocess')){
  class uwpqsfprocess{
	...
	function get_uwqsf_cmf($id, $getcmf){
            ...
            //operatore (ritorna AND)
			$cmfrel = isset($options[0]['cmf']) ? $options[0]['cmf'] : 'AND';

            //$getcmf array dei meta fields
			if(isset($getcmf)){
                ...
                //print_r($output);
                //exit();
                //l'output dovrebbero essere meta key e meta value preparati per wp_query
						return $output;				
	  ...
	  function get_uwqsf_taxo($id, $gettaxo){
            ...
            // $gettaxo è l'array dei generi, in questo primo caso
			if(!empty($gettaxo)){
                //creo l'array con le opzioni
				$taxoperator =array('1'=>'IN','2'=>'NOT IN','3'=>'AND');
				
				foreach($gettaxo as  $v){

                  //in come operatore di default se non è presente nessun operatore
				  $safopt = !empty ( $taxoperator[$v['opt']])  ?  $taxoperator[$v['opt']] : 'IN';

                   //$v['term'] è l' array con tutti i generi selezionati
				   if(!empty($v['term']))	{	
					if( $v['term'] == 'uwpqsftaxoall'){
                      ...
                      //se è un array certo passa di qui
                        ...
                        //genero un array che mi pare possa già essere l'array per la wp_query
					}else{
				    //se $v['term'] esiste ma non è un array
                        $taxo[] = array(
                                'taxonomy' => strip_tags( stripslashes($v['name'])),
                                'field' => 'slug',
                   ...
                   //se $v['term'] è (array con tutti i generi selezionati) è vuoto
			   ...
			   $output = apply_filters( 'uwpqsf_get_taxo', $taxo,$id, $gettaxo );
				...
				unset($output[0]);
                //elimino l'indice di 0 che effettivamente era vuoto
	 ...
	 if($query->is_search()){
        ...
        //$get_tax prepara il get delle tassonomie per essere sparato in wp_query
