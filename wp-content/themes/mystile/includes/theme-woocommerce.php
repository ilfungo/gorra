...
function my_theme_product_tabs( $tabs ) {
  ...
  $content = trim( get_field( 'colori_fiore' ) );
  ...
  if ( !empty( $content ) ) {
    ...
    $tabs[] = array(
      'title' => 'colori_fiore',
      'priority' => 15,
      'callback' => 'my_theme_colori_fiore'
    );
...
function my_theme_colori_fiore() {
  echo get_field( 'colori_fiore' );
}

//			$colori_fiore = get_field('colori_fiore');
//			echo "colori";
//			print_r($colori_fiore);	
*/

// Add ACF Info to Product display page
add_action( 'woocommerce_after_single_product_summary', "ACF_product_content", 10 );
add_action('woocommerce_after_shop_loop_item_title','ACF_product_content', 5);
